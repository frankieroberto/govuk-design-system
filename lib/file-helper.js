    ...
    if (option.isComponent) {
      ...
      if (option.name === 'hint' || option.name === 'label') {
        ...
        if (otherComponentOptions) {
          ...
          option.description += option.description += ` See [${option.name}](#nunjucks-options-table-${option.name}).`
        }
      // Otherwise just link to that component
        ...
        let otherComponentPath = '/components/' + optionName + '#nunjucks-options-example-default'
